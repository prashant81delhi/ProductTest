...
namespace UnitTest
{
    public class Tests
    {
        ...
        {
            private Mock<ProductRepository> _mockRepo;
            private ProductsController _controller;

            [SetUp]
            public void Setup()
            {
                _mockRepo = new Mock<ProductRepository>("products.json"); // Pass mock file path
...
new Product { Id = 1, Name = "Product 1", Color = "Red", Price = 10.5M },
                ...
                var returnValue = actionResult.Value as List<Product>;
                Assert.IsNotNull(returnValue);
                Assert.Equals(2, returnValue.Count);  // Expecting 2 products with the "Red" color
            }
            ...
            public void CreateProduct_ReturnsCreatedAtActionResult_WhenProductIsCreated()
            {
                ...
                Assert.IsNotNull(returnValue);
                Assert.Equals(newProduct.Id, returnValue.Id);
            }
